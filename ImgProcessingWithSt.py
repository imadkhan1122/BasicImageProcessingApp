...
def welcome():
    ...
    st.image('images/hackerShrine.png',use_column_width=True)
    
def photo():
    st.header("Thresholding, Edge Detection and Contours")
    if st.button('See Original Image of Tom'):
        original = Image.open('images/tom.png')
        st.image(original, use_column_width=True)
    image = cv2.imread('images/tom.png')
...
def face_detection():    
    ...
    if st.button('See Original Image'):     
        original = Image.open('images/perry.jpg')
        st.image(original, use_column_width=True)
    image2 = cv2.imread("images/perry.jpg")
